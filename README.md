...
- **Configurable HEIC Handling**: `--jpeg-quality` tunes the HEIC->JPEG encode (1-100, default 95); `--keep-heic` retains the original HEIC alongside the converted JPEG instead of deleting it
...
- **HEIC files**: Converted to JPEG (default quality 95, configurable with `--jpeg-quality`; lossy at any setting) with EXIF preservation; the original HEIC is deleted after a verified conversion by default, or kept alongside it with `--keep-heic`
